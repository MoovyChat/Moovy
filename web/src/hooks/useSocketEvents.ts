...
import { NEST_TYPE } from '../utils/enums';
import { SOCKET_MESSAGE_TYPES } from '../constants';

export const useSocketEvents = (user: Users) => {
     ...
     displayed indicating that the user has rejoined the room. */
      socket.on('rejoin', data => {
        ...
        user &&
          ...
          alert(`${data.user.nickname} re-joined the Nest`);
      });
    ...
    within the same website. */
     ...
     accordingly. */
      ...
      socket.on('join-room-success', data => {
        ...
        data.user.id !== user.id &&
          alert(`${data.user.nickname} Joined the Nest`);
      ...
      socket.on('join-room-error', data => {
        ...
        dispatch(sliceSetNestVisibility(false));
        dispatch(sliceSetNestType(NEST_TYPE.EMPTY));
        data.user.id == user.id && alert(`${data.message}`);
      });
      ...
      socket.on('roomName', data => {
        if (user && data.user.id == user.id) {
          dispatch(sliceSetRoomName(data.room.roomName));
          dispatch(sliceSetJoinedUsers(data.room.users));
          alert(`Joined the '${data.room.roomName}' Nest`);
        }
      });
     ...
     application with the list of users who have joined the current room. */
      ...
      the current user. If it is not the same, it displays a notification using the alert
      ...
      socket.on('userLeft', leftUser => {
        leftUser.id !== user.id && alert(`${leftUser.nickname} Left the Nest`);
      ...
      socket.on('create-room-success', data => {
        ...
        alert(`Created the '${data.roomName}' Nest`);
      });
      ...
      socket.on('play-error', data => {
        user && data.user.id === user.id && alert(data.message);
      });
      ...
      socket.on('kick-user-success', data => {
        const kickedUser = data.kickedUser as Users;
        alert(data.message);
