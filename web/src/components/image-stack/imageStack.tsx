...
const ImageStack: React.FC<props> = ({ followers, following, count }) => {
  ...
  let countText = isFollowerSection
    ? `+${count - 3} more followers`
    : `+${count - 3} more following`;
    ...
    <ImageStackParent>
      <div className='gp'>
        <div className='one profile-box'>
          {users[0] ? (
            <img src={users[0]?.photoUrl} alt='profile' />
          ) : (
            <img
        ...
        <div className='two profile-box'>
          {users[1] ? (
            <img src={users[1]?.photoUrl} alt='profile' />
          ) : (
            <img
        ...
        <div className='three profile-box'>
          {users[2] ? (
            <img src={users[2]?.photoUrl} alt='profile' />
          ) : (
            <img
      ...
      <div className='count'>{count < 3 ? defaultText : countText}</div>
    </ImageStackParent>
  );
};
